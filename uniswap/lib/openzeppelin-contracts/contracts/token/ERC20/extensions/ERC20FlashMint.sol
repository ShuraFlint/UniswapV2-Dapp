...
abstract contract ERC20FlashMint is ERC20, IERC3156FlashLender {
     ...
     */
    //type(uint256).max             uint256 能表示的最大数，保证不会溢出
    ...
    //对于flashLoan来说，msg.sender是一个用户或合约地址，而receiver一定是一个合约地址，并且需要实现onFlashLoan方法
    function flashLoan(
        ...
        //receiver.onFlashLoan是指调用借款者的回调 onFlashLoan
        //借款者在回调里可以使用代币进行操作
        //onFlashLoan需要返回 ERC3156FlashBorrower.onFlashLoan.selector
        if (receiver.onFlashLoan(_msgSender(), token, value, fee, data) != RETURN_VALUE) {
            revert ERC3156InvalidReceiver(address(receiver));
        }
        address flashFeeReceiver = _flashFeeReceiver();
        //借款者需要批准合约将代币转给合约本身，以便合约可以 burn 这些代币
